...
export function sumPublishedPoints(questions: Question[]): number {
    return questions.reduce(
        (currSum: number, question: Question) =>
            currSum + (question.published ? question.points : 0),
        0
    );
}
...
export function makeAnswers(questions: Question[]): Answer[] {
    const answerKey: Answer[] = questions.map(
...
export function sameType(questions: Question[]): boolean {
    ...
    const qCheckType = questions.filter(
        (question: Question): boolean => question.type === qType
    );
    return qCheckType.length === questions.length;
}
...
export function renameQuestionById(
    ...
    const targetQ = questions.map((question: Question) =>
...
export function editOption(
    ...
    const myQs = questions.map((question: Question): Question => {
        if (question.id === targetId) {
            const myOptions = [...question.options];
            if (targetOptionIndex === -1) {
                myOptions.push(newOption);
            } else {
                myOptions[targetOptionIndex] = newOption;
            }
            return { ...question, options: myOptions };
        }
        return question;
    });
    return myQs;
...
export function duplicateQuestionInArray(
    ...
    const myQs = [...questions];
    const targetQIndex: number = questions.findIndex(
        (question: Question): boolean => question.id === targetId
    );
    const dupQ = duplicateQuestion(newId, questions[targetQIndex]);
    myQs.splice(1 + targetQIndex, 0, dupQ);
    return myQs;
}
